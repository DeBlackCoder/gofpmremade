...
function getDailyPhoto(): string {
  const now = new Date();
  const dayOfYear =
    Math.floor((now.getTime() - new Date(now.getFullYear(), 0, 0).getTime()) / 86400000) + 2;
...
export default function CommunityPage() {
  ...
  return (
    <section className="relative w-full min-h-svh overflow-hidden">
      ...
      >
        <Image src={bgUrl} alt="Church community" fill priority quality={90} className="object-cover object-center" />
      </motion.div>
