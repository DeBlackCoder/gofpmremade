...
import type { LiveSettings } from "@/lib/live-data";
    ...
    <div className="flex flex-col min-h-screen px-6 py-8 sm:px-10 sm:py-10 max-w-3xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6 border border-white/10 p-6 backdrop-blur-sm bg-white/3">
        ...
        <label className="flex items-center gap-4 cursor-pointer group">
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <span className="font-body text-white/35 text-xs">Toggle to show the Watch Live button on the homepage</span>
          </div>
        </label>
      ...
      {settings.streamUrl && (
        <div className="mt-8 flex flex-col gap-3">
          <p className="font-body text-white/30 text-[10px] tracking-widest uppercase">Preview</p>
          <a href={settings.streamUrl} target="_blank" rel="noopener noreferrer"
            className="font-body text-white/50 text-xs underline underline-offset-4 hover:text-white transition-colors break-all">
            {settings.streamUrl}
          </a>
        </div>
      )}
