...
function getDailyPhoto(): string {
  const now = new Date();
  const dayOfYear =
    Math.floor((now.getTime() - new Date(now.getFullYear(), 0, 0).getTime()) / 86400000) + 3;
  return `https://images.unsplash.com/${churchPhotos[dayOfYear % churchPhotos.length]}?w=1200&q=80`;
}

const contactDetails = [
  { label: "Address",       value: "14 Grace Avenue, Port Harcourt, Rivers State", href: "https://maps.google.com" },
  { label: "Phone",         value: "+234 801 234 5678",   href: "tel:+2348012345678" },
  { label: "Email",         value: "hello@agchurch.org",  href: "mailto:hello@agchurch.org" },
  { label: "Service Times", value: "Sun 8 AM & 10:30 AM · Wed 6 PM", href: null },
];

type FormState = "idle" | "sending" | "sent" | "error";

const inputClass = "bg-white/10 border-white/20 text-white placeholder:text-white/30 focus-visible:border-white/60 focus-visible:ring-0 rounded-none h-10";
  ...
  return (
    <section className="relative w-full min-h-screen overflow-hidden">
      ...
      <motion.div className="absolute inset-0" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.6 }}>
        <Image src={bgUrl} alt="Church interior" fill priority quality={90} className="object-cover object-center" />
      </motion.div>
      <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/55 to-black/30" />
      <div className="absolute inset-x-0 bottom-0 h-48 bg-gradient-to-t from-black/70 to-transparent" />

      {/* Content */}
      <div className="relative z-10 flex flex-col px-6 py-6 sm:px-10 sm:py-8">
        ...
        >
          ...
          <div className="flex flex-col gap-8">
            <p className="font-body text-white/70 text-sm sm:text-base leading-relaxed max-w-sm">
              We'd love to hear from you. Reach out with any questions, prayer requests, or if you'd simply like to know more about our community.
            </p>
            <dl className="flex flex-col gap-5">
              {contactDetails.map((item, i) => (
                <motion.div
                  ...
                  initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.1 + i * 0.1, duration: 0.6 }}
                >
                  <dt className="font-body text-white/45 text-xs tracking-widest uppercase mb-1">{item.label}</dt>
                  <dd>
                    {item.href ? (
                      <a
                        href={item.href}
                      ...
                      <span className="font-body text-white font-semibold text-sm sm:text-base">{item.value}</span>
                    )}
                  </dd>
                </motion.div>
              ))}
          ...
          <div className="backdrop-blur-md bg-white/10 border border-white/15 rounded-2xl p-6 sm:p-8">
            {status === "sent" ? (
              ...
              >
                <span className="font-body text-white/40 text-xs tracking-widest uppercase">Message sent</span>
                <p className="font-heading text-white font-black text-3xl sm:text-4xl leading-tight">
                  Thank you,<br />{form.name.split(" ")[0] || "friend"}.
                </p>
                <p className="font-body text-white/65 text-sm leading-relaxed max-w-xs">
                  We'll get back to you as soon as we can. May God bless your day.
                </p>
                <button
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1">
                    <label className="font-body text-white/45 text-xs tracking-widest uppercase">Name</label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1">
                    ...
                    <Input name="phone" type="tel" value={form.phone} onChange={handleChange} placeholder="+234 800 000 0000" className={inputClass} />
                  </div>
                  <div className="flex flex-col gap-1">
                ...
                {/* Message */}
                <div className="flex flex-col gap-1">
                  <label className="font-body text-white/45 text-xs tracking-widest uppercase">Message</label>
                  <Textarea name="message" value={form.message} onChange={handleChange} required rows={5} placeholder="Write your message here…" className="bg-white/10 border-white/20 text-white placeholder:text-white/30 focus-visible:border-white/60 focus-visible:ring-0 rounded-none resize-none" />
                </div>

                {/* Error */}
                {status === "error" && (
                  <p className="font-body text-red-400 text-sm">Something went wrong. Please try again.</p>
                )}
