...
export default function EventsPage() {
  ...
  const { data: paginatedEvents, isLoading, error } = useEvents(page);
  ...
  const handleRegister = async (eventId: string) => {
    try {
      ...
      addToast({ type: "error", message: error?.message || "Failed to register" });
    }
  };
          ...
          >
            <AnimatePresence mode="popLayout">
              {filtered.map((event: any, i) => {
                ...
                const dayName = eventDate.toLocaleString("en", { weekday: "short" });
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center leading-none pt-0.5">
                        <span className="font-body text-white/40 text-[10px] tracking-widest uppercase">{month}</span>
                        <span className="font-heading text-white font-black text-3xl sm:text-4xl leading-none mt-0.5">{day}</span>
                        <span className="font-body text-white/40 text-[10px] tracking-widest uppercase mt-0.5">{dayName}</span>
                      </div>
